...
class AuthCodeRequestTest {
  ...
  void noArgsConstructorAndSettersShouldPopulateRequest() {
    ...
    assertThat(request.toString()).contains("chat-1", "123456", "user-1", "user@example.com");
  }

  @Test
  void shouldUseLombokValueSemantics() {
    AuthCodeRequest first = new AuthCodeRequest("chat-1", "123456", "user-1", "user@example.com");
    AuthCodeRequest same = new AuthCodeRequest("chat-1", "123456", "user-1", "user@example.com");
    AuthCodeRequest different = new AuthCodeRequest("chat-2", "123456", "user-1", "user@example.com");
  ...
  void shouldHandleNullFieldsInGeneratedMethods() {
    ...
    AuthCodeRequest sameEmpty = new AuthCodeRequest(null, null, null, null);

    assertThat(empty).isEqualTo(sameEmpty);
    assertThat(empty.hashCode()).isEqualTo(sameEmpty.hashCode());
    assertThat(empty.toString()).contains("chatId=null", "code=null", "userId=null", "email=null");
  }
}
